...
# FIJAMOS EL PUERTO SEGÚN TU CONFIGURACIÓN
PORT = 7777 
...
class AppState:
    ...
    def add_message(self, sender, text, is_me=False, is_sys=False):
        ...
        # Lógica de "hablar" para la animación
        if not is_sys:
...
# ==========================================
# 3. LÓGICA DE RED (ADAPTADA DE TU MAIN_V1.PY)
# ==========================================
def get_best_ip():
...
class ChatClient:
    ...
    # --- PERSISTENCIA (Igual que main_v1) ---
    ...
    # --- LÓGICA DE PAQUETES (Base de main_v1 adaptada a GUI) ---
    def on_packet(self, packet, addr):
        ...
        if packet.msg_type == MSG_DISCOVERY:
            ...
            # Comprobar si ya existe en peers
            ...
            if ip not in known_ips:
                 ...
                 if not self.target_ip:
                     ...
                     STATE.add_message("SYS", f"Nuevo contacto: [{pid}] {nombre}", is_sys=True)
            return

        # 2. HANDSHAKE (HELLO)
        if packet.msg_type == MSG_HELLO:
            # CASO A: Ya tenemos sesión (Lógica corregida de main_v1)
            if ip in self.sessions:
                try:
                    self.sessions[ip].perform_handshake(packet.payload, is_initiator=True)
                    self.save_sessions_to_disk()
                    STATE.add_message("SYS", f"✅ Handshake actualizado con {ip}", is_sys=True)
                except Exception as e:
                    STATE.add_message("SYS", f"❌ Error Handshake {ip}: {e}", is_sys=True)
                return
        ...
        # 3. CHAT (DATA)
            ...
            if ip in self.sessions:
                try:
                    ...
                    STATE.add_message("SYS", f"♻️ Clave caducada con {ip}. Renegociando...", is_sys=True)
                    del self.sessions[ip]
                    self.save_sessions_to_disk()
                    self.connect_manual(ip, "Desconocido")
            else:
                STATE.add_message("SYS", f"⚠️ Mensaje ilegible de {ip}. Reconectando...", is_sys=True)
                self.connect_manual(ip, "Desconocido")

    # --- COMANDOS (Traducción de input GUI a lógica main_v1) ---
    def process_command(self, text):
        ...
        if text == "/list":
            STATE.add_message("SYS", "--- Mira la lista a la izquierda ---", is_sys=True)
            return
    ...
    def connect_manual(self, ip_target, name_target):
        ...
        STATE.add_message("SYS", f"⏳ Enviando solicitud a {name_target}...", is_sys=True)
        session = SessionCrypto(self.key_manager.static_private)
    ...
    def accept_connection(self, peer_id):
        ...
        if ip not in self.pending_requests:
            STATE.add_message("SYS", f"⚠️ {name} no tiene solicitud pendiente.", is_sys=True)
            return
    ...
    def send_chat(self, text):
        if self.target_ip and self.target_ip in self.sessions:
            try:
                ...
                STATE.add_message("SYS", f"❌ Error enviando: {e}", is_sys=True)

# ==========================================
# 4. INTERFAZ GRÁFICA (CODEC DISPLAY CON SIDEBAR)
# ==========================================
class CodecDisplay:
    ...
    def draw(self, screen, font_chat, font_ui):
        ...
        y_contact = sidebar_rect.top + 45
        if not STATE.peers:
            screen.blit(font_chat.render("Scanning...", True, (30, 80, 30)), (sidebar_rect.left + 10, y_contact))
        else:
            for pid, data in STATE.peers.items():
                name = data['name']
                is_target = (name == STATE.target_name)
                color_c = (200, 255, 200) if is_target else (50, 180, 50)
                prefix = "► " if is_target else ""
                
                # Candado visual si hay sesión activa con esta IP
                secure = ""
                # Accedemos a las sesiones a través de una comprobación segura (simulada aquí por estado visual)
                if is_target and "CONECTADO" in STATE.status_msg: secure = " [🔐]"
                
                txt_str = f"{prefix}[{pid}] {name}{secure}"
                screen.blit(font_chat.render(txt_str, True, color_c), (sidebar_rect.left + 10, y_contact))
                y_contact += 25
...
async def main_loop(client):
    ...
    pygame.display.set_caption("MGS SECURE LINK - v2")
    global CHARS
